...
import org.delcom.services.BorrowingService
...
fun Application.configureRouting() {
    ...
    val borrowingService: BorrowingService by inject()
    ...
    routing {
        get("/") {
            call.respondText("API Manajemen Perpustakaan - pam-2026-ifs23015-proyek1-be")
        }
        ...
        authenticate(JWTConstants.NAME) {
            ...
            // Route Borrowings
            route("/borrowings") {
                get { borrowingService.getAll(call) }
                post { borrowingService.post(call) }
                get("/summary") { borrowingService.getSummary(call) }
                get("/{id}") { borrowingService.getById(call) }
                put("/{id}") { borrowingService.put(call) }
                delete("/{id}") { borrowingService.delete(call) }
            }
        }
